	...
	*/
	...
	for (const auto& testPoint : X_test) 
    {
        ...
        for (int i = 0; i < X_train_.size(); i++) 
        {
			...
			//double dist = SimilarityFunctions::hammingDistance(testPoint, X_train_[i]);
			double dist = SimilarityFunctions::manhattanDistance(testPoint, X_train_[i]);
            //double dist = SimilarityFunctions::minkowskiDistance(testPoint, X_train_[i],3);
        ...
        if (predictedLabel == -1) 
        {
			throw std::runtime_error("Error: Invalid predicted label.");
		}
