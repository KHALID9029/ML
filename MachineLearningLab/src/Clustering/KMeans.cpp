...
void KMeans::fit(const std::vector<std::vector<double>>& data) {
	...
	*/
	...
	// TODO
	int num_smaples = data.size();
	int num_features = data[0].size();

	int max_val = normalizedData.size();
	...
	for (int i = 0; i < numClusters_; i++) 
	{
		...
		while (find(selectedIndices.begin(), selectedIndices.end(), index) != selectedIndices.end())
		{
			index = rand() % max_val;
		}
		selectedIndices.push_back(index);
		centroids_.push_back(normalizedData[index]);
	}

	vector<int> assignments(num_smaples, -1);
	bool converged = false;

	for (int it = 0; it < maxIterations_ && !converged; it++)
	{
		converged = true;
		vector<vector<double>> newCentroids(numClusters_, vector<double>(num_features, 0.0));
		vector<int> clusterCounts(numClusters_, 0);
		...
		for (int i = 0; i < num_smaples; i++)
		{
			...
			{
				double dist = SimilarityFunctions::euclideanDistance(normalizedData[i], centroids_[j]);
				if (dist < minDist)
				{
					minDist = dist;
					closestCentroid = j;
			...
			{
				newCentroids[closestCentroid][j] += normalizedData[i][j];
			}
		}
...
std::vector<int> KMeans::predict(const std::vector<std::vector<double>>& data) const {
    ...
    */
	...
	for (int i = 0;i < num_samples;i++)
	{
		...
		{
			double dist = SimilarityFunctions::euclideanDistance(data[i], centroids_[k]);
			if (dist < min_dist)
			{
				min_dist = dist;
				closest_centroid = k;
		...
		labels.push_back(closest_centroid + 1);
	}
